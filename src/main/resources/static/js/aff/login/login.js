...
$(function () {
  ...
  if (params.has('error')) {
    ...
    let message;
    switch (code) {
      case '0': message = '2024년도 직원근무평가 대상직원이 아닙니다.'; break;
      ...
      case '5': {
        ...
        $('#modalOkBtn').attr('onclick', `location.href='/aff/pwd/${idx}'`);
        break;
      }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        // 서버 result코드 활용해도 되고, 단일 메세지로도 OK
        let message = '아이디 또는 비밀번호가 올바르지 않습니다.';
        // 필요 시 코드별 분기
        if (data && data.result === '5' && data.idx) {
          message = '현재 비밀번호가 설정되어 있지 않습니다. 비밀번호 설정 페이지로 이동합니다.';
          document.getElementById('modalOkBtn')
            ?.setAttribute('onclick', `location.href='/aff/pwd/${data.idx}'`);
        }
        openPopup(message);
        return;
      }
