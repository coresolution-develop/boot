...
import com.coresolution.pe.service.AffBankService;
import com.coresolution.pe.service.AffKpiInfoGeneralExcelService;
...
public class AffExcelContoller {
    ...
    @Autowired
    private AffBankService bankService;
    @Autowired
    private AffKpiInfoGeneralExcelService kpiInfoGeneralExcelService;

    @Value("${app.admin.sub-code}") // application.yml 등에 관리자의 sub_code 를 선언해 두시면 편합니다.
    private String adminSubCode;

    // private final PESideService pesService;

    // ───────────────────────────────────────────────────────────
    // 1) 직원 템플릿 다운로드 (GET /aff/excel/downloadTemplate)
    // ───────────────────────────────────────────────────────────
        ...
        try (XSSFWorkbook wb = new XSSFWorkbook()) {
            ...
            Object[][] samples = {
                {"효사랑전주요양병원", "사랑모아", "간호부", "100001", "간호사", "홍길동",
                 "2023-01-01", "", "010-1234-5678", "TRUE", "FALSE", "FALSE", "FALSE"},
                {"효사랑가족요양병원", "효사랑가족", "원무부", "100002", "원무팀장", "김철수",
                 "2020-07-15", "2025-12-31", "010-9876-5432", "FALSE", "FALSE", "TRUE", "FALSE"}
            };
            ...
            // ── TRUE/FALSE 컬럼에 드롭다운 데이터 검증 (K~N열 = index 10~13) ──
            DataValidationHelper dvHelper = sheet.getDataValidationHelper();
            DataValidationConstraint dvConstraint =
                dvHelper.createExplicitListConstraint(new String[]{"TRUE", "FALSE"});
            org.apache.poi.ss.util.CellRangeAddressList boolRange =
                new org.apache.poi.ss.util.CellRangeAddressList(2, 200, 10, 13);
            DataValidation dv = dvHelper.createValidation(dvConstraint, boolRange);
            dv.setShowErrorBox(true);
            dv.createErrorBox("입력 오류", "TRUE 또는 FALSE만 입력 가능합니다.");
    ...
    public String confirmUpload(@RequestParam("year") int year,
        ...
        tableInitService.ensureAffTablesExist(year);
    ...
    public String subuploadDepartments(
        ...
        try (InputStream is = file.getInputStream();
            ...
            model.addAttribute("year", year);
            return "Excel/aff/departmentPreview"; // 미리보기용 뷰
        }
    }
    ...
    public String resetSubManagement(@RequestParam("year") int year,
        ...
        return "redirect:/aff/admin/subManagement?year=" + year;
    }
    ...
    public String confirmUpload(HttpSession session, RedirectAttributes ra) {
        ...
        if (rows == null || year == null) {
            ...
            return "redirect:/aff/admin/evaltemplate/uploadStatus?year=" + (year == null ? 0 : year);
        }
        ...
        return "redirect:/aff/admin/evaltemplate/uploadStatus?year=" + year;
    }
    ...
    public String evaluploadStatus(@RequestParam("year") int year, Model model) {
        model.addAttribute("year", year);
        return "Excel/aff/EvaluploadStatus";
    }

    // ───────────────────────────────────────────────────────────
    /** 1) 부서 엑셀 템플릿 다운로드 */
    @GetMapping("/kpi/general/template")
    public void kpidownloadTemplate(HttpServletResponse response) throws IOException {
        String fileName = "department_template.xlsx";
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader(
                "Content-Disposition",
        ...
        try (var workbook = new XSSFWorkbook()) {
            var sheet = workbook.createSheet("kpi템플릿");
