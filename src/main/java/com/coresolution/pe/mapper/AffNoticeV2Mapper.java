...
@Mapper
public interface AffNoticeV2Mapper {
  ...
  @Select("""
      ...
      FROM personnel_evaluation_aff.notice_v2
      WHERE is_active = 1
  ...
  @Select("""
      ...
      ORDER BY pinned DESC, sort_order ASC, id DESC
      """)
  List<NoticeV2> findActiveaff(@Param("evalYear") int evalYear);
  ...
  @Select("""
        SELECT id,
               ...
               created_at    AS createdAt,
               updated_at    AS updatedAt
        FROM personnel_evaluation_aff.notice_v2 WHERE id = #{id}
      """)
  NoticeV2 findById(@Param("id") int id);

  @Insert("""
        INSERT INTO personnel_evaluation_aff.notice_v2
  ...
  @Update("""
        UPDATE personnel_evaluation_aff.notice_v2 SET
  ...
  @Delete("DELETE FROM personnel_evaluation_aff.notice_v2 WHERE id=#{id}")
