...
@Mapper
public interface AffKpiInfo2025Mapper {

  /**
   * 연도 단위 전체 삭제 (필요 시)
   */
  @Delete("""
      DELETE FROM personnel_evaluation_aff.kpi_personal_2025
   ...
   */
  @Insert("""
      INSERT INTO personnel_evaluation_aff.kpi_personal_2025 (
        eval_year,
        hospital_name,
        emp_id,
        total_score,
  ...
  @Select("""
      WITH
        ...
        joined_multi AS (
          ...
          JOIN personnel_evaluation_aff.users_2025 eu
      ...
      FROM personnel_evaluation_aff.kpi_personal_2025 p
      JOIN personnel_evaluation_aff.users_2025 u
        ON u.eval_year = p.eval_year
       AND u.id       = SUBSTRING_INDEX(p.emp_id, '.', 1)
      LEFT JOIN personnel_evaluation_aff.sub_management s
        ON s.sub_code = u.sub_code
       AND s.eval_year = u.eval_year
      JOIN personnel_evaluation_aff.team t
  ...
  @Insert("""
        INSERT INTO personnel_evaluation_aff.kpi_eval_result (
        eval_year,
        hospital_name,
        emp_id,
        total_score,
  ...
  @Select("""
        ...
        SELECT
            ...
            ROUND(
              AVG(
                CASE
                  WHEN s.data_ev='A' AND s.data_type='AC' AND s.answered_count=11 THEN s.total_score
                  WHEN s.data_ev='B' AND s.data_type IN ('AD','AE') AND s.answered_count=21 THEN s.total_score
                  ELSE NULL
                END
              )
            , 1) AS eval100
        ...
        WHERE s.eval_year = #{year}
          ...
          AND s.del_yn    = 'N'
          AND (
            (s.data_ev='A' AND s.data_type='AC' AND s.answered_count=11)
            OR
            (s.data_ev='B' AND s.data_type IN ('AD','AE') AND s.answered_count=21)
          )
        GROUP BY s.target_id
            ...
            SELECT
              ...
              e.eval100 AS eval100,

              /* 다면 70점 환산 */
              CASE
                WHEN e.eval100 IS NULL OR e.eval100 = 0 THEN NULL
                ELSE ROUND(e.eval100 * 0.70, 2)
              END AS evalSum70,

              /* 총점(10/10/10/70) */
              ROUND(
                  COALESCE(CAST(NULLIF(TRIM(k.kcol08), '') AS DECIMAL(10,2)), 0) * 0.10
                + COALESCE(CAST(NULLIF(TRIM(k.kcol12), '') AS DECIMAL(10,2)), 0) * 0.10
                + COALESCE(CAST(NULLIF(TRIM(k.kcol16), '') AS DECIMAL(10,2)), 0) * 0.10
                + COALESCE(e.eval100, 0) * 0.70
              , 2) AS totalCalc,
            ...
            FROM personnel_evaluation_aff.users_2025 u
            JOIN personnel_evaluation_aff.kpi_info_general_2025 k
              ON k.kcol02 = u.id
            LEFT JOIN personnel_evaluation_aff.kpi_eval_result_staff kers
            ...
            ORDER BY
              CASE WHEN #{order} = 'EVAL'  THEN COALESCE(e.eval100,0) END DESC,
              CASE WHEN #{order} = 'TOTAL' THEN totalCalc            END DESC,
  ...
  @Select("""
      ...
      JOIN personnel_evaluation_aff.kpi_info_general_2025 k
        ON k.kcol02 = u.id
      LEFT JOIN personnel_evaluation_aff.kpi_eval_result ker
  ...
  @Insert("""
      INSERT INTO personnel_evaluation_aff.kpi_eval_result_staff
      (
  ...
  @Update("""
      WITH score_groups AS (
        ...
        FROM personnel_evaluation_aff.kpi_eval_result r
      ...
      UPDATE personnel_evaluation_aff.kpi_eval_result r
      JOIN ranked x
  ...
  @Update("""
      WITH base AS (
        SELECT
          ...
          emp_id,
          total_score
        FROM personnel_evaluation_aff.kpi_eval_result_staff
        WHERE eval_year = #{year}
      ...
      UPDATE personnel_evaluation_aff.kpi_eval_result_staff ker
      JOIN graded g
      ...
      es_pivot AS (
        ...
        GROUP BY targetId
          ...
          WHERE u.eval_year = #{year}
            ...
            -- AND NOT EXISTS (
            --     SELECT 1
            --     FROM personnel_evaluation_aff.user_roles_2025 r
  ...
  @Update("""
      INSERT INTO personnel_evaluation_aff.kpi_eval_clinic
      (
        eval_year, hospital_name, emp_id,
      ...
      WITH es AS (
        ...
        FROM personnel_evaluation_aff.evaluation_submissions s
        JOIN personnel_evaluation_aff.users_2025 tgt
