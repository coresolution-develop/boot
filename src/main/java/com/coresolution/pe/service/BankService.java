    ...
    public UploadResult importExcel(MultipartFile file, int year, String mode /* UPSERT | REPLACE */)
        ...
        try (XSSFWorkbook wb = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int r = firstRow; r <= sheet.getLastRowNum(); r++) {
                ...
                String d2 = fmt.formatCellValue(row.getCell(2)).trim().toUpperCase(); // 유형(C)
                ...
                if (!(d2.equals("AA") || d2.equals("AB"))) {
                    errors.add("Row " + rownum + ": 문제유형(d2)은 AA/AB만 허용");
                    continue;
                }
    ...
    public ParseResult parseExcel(MultipartFile file) throws IOException {
        ...
        try (XSSFWorkbook wb = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int r = 2; r <= sheet.getLastRowNum(); r++) {
                ...
                String d2 = fmt.formatCellValue(row.getCell(2)).trim().toUpperCase();
                ...
                if (!(d2.equals("AA") || d2.equals("AB")))
                    rowErr.add("문제유형(d2)은 AA/AB만 허용");
